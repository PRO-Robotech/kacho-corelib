// Package authz — listobjects.go (KAC-127 Phase 4).
//
...
// # Cache invalidation
//
//   - Cache TTL = 5s positive-only (acceptance §4.4 D-2).
...
//     в кэше — оно возвращается (graceful degradation, acceptance §5.4).
...
//     НЕ PermissionDenied (acceptance §3 D-4).
//
...
type ListObjectsRequest struct {
	...
	MaxResults   uint32
	PageToken    string
	// AuthzModelID — pinned authorization_model_id (acceptance D-12).
	// Empty → server default. Передаётся через ctx-metadata (см. impl).
	AuthzModelID string
}
...
type ListObjectsConfig struct {
	...
	// AuthzModelID — pinned FGA authorization_model_id (acceptance D-12).
	// Передаётся на каждый Request.
	AuthzModelID string
...
// client == nil → service всё равно создаётся, но любой ListAllowedIDs вернёт
...
//   - len(ids) == 0 → возвращает ([], nil) — caller вернёт empty response.
...
//
// Acceptance §4.4: principal type/id ужé encoded в subject string FGA-style
...
//
// Двухуровневая структура даёт O(1) invalidateBySubject (one delete on
// outer map).
//
...
func (c *listObjectsCache) evictIfNeededLocked() {
	...
	// Сортировка по exp ascending — берём первые `toEvict`. Insertion-sort
