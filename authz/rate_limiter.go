...
// rateLimiter — token-bucket per-Principal на denied-storm protection (I10
// в acceptance §8 Risks). При flooding `GET /vpc/v1/networks/*` от
// unauthorized user negative cache отсутствует (D-9 negative-not-cached) →
// каждый запрос идёт в `kacho-iam.Check` → потенциальный DoS на kacho-iam.
//
// Token-bucket per-Principal даёт верхнюю границу rate'а Check'ов от одного
...
// newRateLimiter создаёт лимитер. ratePerSec ≤ 0 → disabled (Allow всегда true).
