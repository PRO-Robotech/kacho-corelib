...
type InterceptorOptions struct {
	...
	// DenyRateLimitPerSec — token-bucket per-Principal на denied storm (I10).
	...
	// `operations.PrincipalFromContext(ctx)` (E2-corelib). Можно переопределить
	// для тестов.
...
// stream'а. Дальше идёт обычный wrapping.
//
...
func (i *Interceptor) Stream() grpc.StreamServerInterceptor {
	return func(srv any, ss grpc.ServerStream, info *grpc.StreamServerInfo, handler grpc.StreamHandler) error {
		// Stream req не доступен до первого Recv → для stream-RPC передаём nil как req.
...
func (i *Interceptor) authorize(ctx context.Context, fullMethod string, req any) (Decision, error) {
	...
	if !ok {
		// 2a. Эвристический фолбэк — если RPC явно internal (по имени), пропускаем.
		// Это не security-bypass: internal RPC экспонируются на :9091 без external
		// auth-interceptor'а; на public listener'е они не достижимы. Если RPC
		// случайно попал на public listener — fail-closed (см. ниже).
		if methodIsInternal(fullMethod) {
	...
	// 6. Rate-limit denied-storm (I10): применяется ДО Check call, иначе
...
// Metrics — счётчики для Prometheus / тестов. Lock-free.
type Metrics struct {
	Allowed     uint64
	Denied      uint64
	Unavailable uint64
...
// Metrics возвращает snapshot счётчиков.
...
// buckets, у которых lastSeen старше maxAge. Вернёт кол-во удалённых.
