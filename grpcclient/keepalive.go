...
// keepalive (KAC-244). Единая точка истины для keepalive-параметров inter-service
...
// проактивно обнаруживают мёртвый conn и переустанавливают его.
//
...
const (
	// DefaultKeepaliveTime — интервал ping'а (FD-1: агрессивный 10s, чтобы поймать
	// half-open до следующего всплеска в kind, где idle-flow умирает быстрее 30s).
	DefaultKeepaliveTime = 10 * time.Second
	// DefaultKeepaliveTimeout — ack-deadline = треть интервала (FD-1, инвариант
	// «таймаут = треть Time»).
...
// пинги держат conn тёплым даже без активных стримов — прямо лечит half-open-столл.
