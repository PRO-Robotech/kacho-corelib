...
// TestNewServer_AcceptsIdleKeepalive — KA-04b (gold behavioral, reproducible).
//
...
// Idle-conn с keepalive-пингами держится тёплым весь idle-период (> 2
// ping-интервалов): conn остаётся READY, GOAWAY too_many_pings не приходит,
// RPC после простоя проходит немедленно. Это прямое моделирование фикса KAC-244 —
// idle-prone conn (authz/drainer) переживает простой без half-open-столла.
...
// TestNewServer_IdleStreamSurvives — KA-04b (вариант с активным idle-стримом).
//
