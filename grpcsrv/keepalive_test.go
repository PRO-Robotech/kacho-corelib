...
// корень бага KAC-244 со стороны сервера.
const (
	grpcDefaultMinTime             = 5 * time.Minute
	grpcDefaultPermitWithoutStream = false
	clientKeepaliveTime            = 10 * time.Second // FD-1
)

// TestDefaultKeepaliveEnforcement — KA-04a: сервер допускает idle keepalive-пинги
...
// TestDefaultEnforcement_FixesGrpcDefault — детерминированный policy-level
// RED→GREEN. Доказывает, что DefaultKeepaliveEnforcement ОТЛИЧАЕТСЯ от gRPC-дефолта
// в обе стороны, нужные для приёма idle-пингов стандартного клиента (Time=10s):
//
...
//
// Тест красный, если DefaultKeepaliveEnforcement вернёт gRPC-дефолт (= нет фикса).
