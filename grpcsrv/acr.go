// Package grpcsrv — acr.go (sub-phase 5.4): the SHARED ACR (Authentication
// Context Class Reference) ranking used by BOTH the api-gateway public step-up
// gate and the kacho-iam internal acr-floor, so the two never drift.
//
// ACR ordering (normative — sub-phase 3.2 §2.1):
//
...
// (alongside x-kacho-principal-*). It is read ONLY under the FD-4 trust
// invariant (see UnaryTrustedPrincipalExtract) — on an unverified peer it is
// dropped with the principal (anti-spoof).
