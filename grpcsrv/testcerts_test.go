...
// testcerts_test.go — ephemeral test-CA + cert generation for SEC-B mTLS bufconn
// tests. NOTHING here is committed cert material: every CA/leaf is generated in
// memory at test time (acceptance §«Примечания по реализму»: ban secrets-in-git;
// test cert material is generated, not stored). Written to a per-test temp dir so
// the corelib TLS-helpers (which read files) can load them.
