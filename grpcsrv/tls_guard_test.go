...
// tls_guard_test.go — SEC-B-19 anti-regression guard (FD-7): inter-service gRPC
// TLS-credentials are built ONLY through the corelib helpers
// (grpcsrv.TLSServerCreds / grpcclient.TLSClientCreds). A bare
// credentials.NewTLS(...) or a hand-rolled tls.Config anywhere in corelib
// production code (outside the two helper files) turns this guard RED.
//
// Scope: production code only (*_test.go excluded — test-CA/cert generation is
// allowed to use crypto/tls / x509 directly, per acceptance SEC-B-19 allow-list).
...
func TestSECB19_NoDirectTLSConfigOutsideHelper(t *testing.T) {
	...
	require.Empty(t, violations,
		"TLS-credentials must be built only via grpcsrv.TLSServerCreds / grpcclient.TLSClientCreds (FD-7, SEC-B-19); offenders: %v",
		violations)
}
