...
// acr_extract_test.go — sub-phase 5.4: acr-carrying trusted-principal extract.
//
...
// into ctx ONLY under the same FD-4 trust invariant as the principal: trusted ⟺
// the peer is mTLS-verified. On an unverified TLS peer the forwarded acr is
// DROPPED with the principal (anti-spoof — 5.4-06 plumbing half).
//
...
// --- 5.4 corelib: ACR ranking helper (shared with gateway StepUpGate / iam floor).
...
// --- 5.4 corelib: ACRSatisfies — required=="" / "0" is a no-op (always satisfied),
//
...
// --- 5.4 corelib: trusted peer carries acr into ctx.
...
// --- 5.4-06 (plumbing half): unverified TLS peer ⇒ acr dropped (not trusted).
func TestTrustedACR_UnverifiedPeer_Dropped(t *testing.T) {
	...
	_, err := chained(ctx, nil, nil, final)
	require.NoError(t, err)
	require.False(t, gotTrusted, "unverified TLS peer ⇒ acr NOT trusted (FD-4)")
	require.Equal(t, "", gotACR, "spoofed acr from unverified peer must be dropped")
}

// --- 5.4 corelib: insecure (dev) listener ⇒ acr accepted as today (back-compat),
//
...
// --- 5.4 corelib: a ctx that never went through the extract reports no acr.
...
// --- 5.4 corelib: WithTrustedACR test-support helper round-trips acr+trusted.
