...
// cert_identity_bufconn_test.go — SEC-B identity-extractor + trust-invariant over
// real mTLS (bufconn). Verifies SEC-B-12 (SAN→identity downstream), SEC-B-15
// (principal trusted under verified cert), SEC-B-16 (principal NOT trusted without
// verified cert: handshake-reject transitively), SEC-B-17 (insecure listener
// accepts principal as today).
...
// serveBufconnNet starts a TLS-or-insecure server with the SEC-B interceptor
...
// --- SEC-B-12 + SEC-B-15: verified client-cert ⇒ cert-identity extracted downstream
//     AND principal-metadata trusted (both available, orthogonal, FD-4).
func TestSECB12_15_VerifiedCert_IdentityAndPrincipalTrusted(t *testing.T) {
	...
	require.Equal(t, testClientSAN, certID, "cert-identity must be the verified SAN (SEC-B-12)")
	require.True(t, verified, "peer must be mTLS-verified")
	require.True(t, trusted, "principal-metadata trusted under verified cert (SEC-B-15)")
	require.Equal(t, "usr-alice", p.ID, "principal carried downstream alongside cert-identity (FD-4)")
}

// --- SEC-B-16: without a verified client-cert on a require-and-verify listener the
...
// --- SEC-B-17: insecure listener accepts principal as today (dev backward-compat).
func TestSECB17_InsecureListener_AcceptsPrincipal(t *testing.T) {
	...
	require.True(t, trusted, "insecure dev-listener accepts principal as today (FD-1/FD-4 invariant N/A)")
