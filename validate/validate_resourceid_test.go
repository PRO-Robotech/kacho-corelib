...
// of that family 400 at the api-gateway authz edge (gh kacho-api-gateway#73).
//
...
//   - IAM domain prefixes — mirrored as string literals from
//     kacho-iam/internal/domain/constants.go (corelib is UPSTREAM of kacho-iam
//     in the build graph and `internal/` is not importable, so the IAM prefixes
//     cannot be imported; they are duplicated here verbatim with this comment as
//     the single source-of-truth pointer). cag/cond/soc/evt
//     (constants_extended.go) are deliberately excluded: they use the
...
var allIDPrefixes = func() map[string]string {
	m := map[string]string{
		...
		// kacho-iam/internal/domain/constants.go (mirrored literals)
...
// TestResourceID_GuardEveryLivePrefixIsKnown is the regression guard requested
// in the gh#73 fix design: a future new resource/operation prefix that lands in
// kacho-corelib/ids (or a new IAM resource) must also be registered in
// `resourceIDPrefixes`, otherwise every well-formed id of that family would be
// rejected with InvalidArgument (400) at the gateway authz edge instead of
// reaching the owner service.
