...
// (sub-phase 1.4 D-8).
//
// Problem it replaces: today a service whose register-drainer / IAM-peer is down
// silently logs a Warn and keeps accepting Creates — every resource created in
// that window gets no owner-tuple (the exact "ресурс создан, tuple обвалился"
// failure the epic exists to kill). The gate turns that silent loss into an
// explicit refusal (fail-closed is safer than a grant-leak).
//
// Single canonical mode (N5): when --require-iam (KACHO_<SVC>_REQUIRE_IAM=true)
...
// retryable, fail-closed condition per data-integrity.md §cross-domain).
