...
// Unit tests for the error-classification function (sub-phase 1.4, D-5).
//
// RED-phase: `drainer.Classify` / `drainer.Class*` do not yet exist, so this
// file fails to compile (test-first, ban #12). The classifier is the single,
// testable decision point that decides whether an apply-error poisons a row
// (permanent) or retries unbounded with backoff (transient).
//
// Acceptance scenarios: 1.4-04 (transient long-outage NO-poison), 1.4-05
// (permanent → poison). The classifier underpins both — transient gRPC
// Unavailable / DeadlineExceeded / connection-refused / timeout must classify
// as ClassTransient (never poison); only ErrPermanent (and decode-fail, handled
// separately) classifies as ClassPermanent.
...
func TestClassify_TransientNeverPoisons(t *testing.T) {
	...
	transient := []error{
		...
		// poison — BLOCKER-1 1.4-01b). Only InvalidArgument is permanent.
