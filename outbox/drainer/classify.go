...
//
// Sub-phase 1.4 D-5 ("transient-class no-poison"): a long-but-transient IAM
// outage (gRPC Unavailable / DeadlineExceeded / connection-refused / timeout)
// must NEVER poison a row — it retries forever (with backoff) so the owner-tuple
// is never lost to a temporary peer outage. Only permanent errors (4xx-non-409,
...
// (BLOCKER-1, acceptance 1.4-01b). A raw, un-wrapped error of unknown shape is
...
// peer (e.g. apps-SA fga_writer grant not yet applied — 1.4-01b) which heals
// without poisoning. Appliers wrap genuinely-permanent cases in ErrPermanent.
