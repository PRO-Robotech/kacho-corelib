...
//     Соответствует CLAUDE.md §запрет #10 (within-service refs DB-level).
//   - **Exp-backoff retry**: transient errors (всё, что не [ErrAlreadyApplied]
...
//     (использует detached ctx с ApplyTimeout grace), затем exit. См. W1.1-11.
...
// (OQ-W1.1-6). Hijack необходим, потому что LISTEN-state не выживает pool's
// idle-conn recycle. Conn закрывается при reconnect / shutdown.
//
...
// # См. также
//
//   - Acceptance-doc (the spec): docs/specs/sub-phase-W1.1-fga-outbox-drainer-acceptance.md
//   - Writer-side (Emit / WriteEvent): пакет github.com/PRO-Robotech/kacho-corelib/outbox
//   - Vault: obsidian/kacho/packages/corelib-outbox-drainer.md
package drainer
