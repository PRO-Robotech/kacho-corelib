...
// Integration test for the transient head-of-line starvation regression
// introduced by sub-phase 1.4 D-5 (transient-class no-poison).
//
// Background / root cause
//
//	D-5 made markTransientFailure cap attempt_count at MaxAttempts-1 (LEAST),
//	so a persistently-transient-failing row stays claimable FOREVER
//	(attempt_count < MaxAttempts is always true). Combined with the claim's
//	`ORDER BY id`, a set of low-id rows that keep failing transiently
//	permanently SHADOW any higher-id row: the small per-claim batch limit
//	(1..4) plus the retry-backoff means the claim re-selects the same stuck
//	low-id rows on every wake-up and never advances to the fresh higher-id
//	intent. The new intent is NEVER delivered → at-least-once is violated
//	under a sustained outage (real failure: kacho-iam A-07).
//
...
//
// RED (pre-fix): the fresh row's sent_at stays NULL forever → fails.
// GREEN (post-fix): the fresh row is delivered while the stuck rows keep
// retrying.
...
func Test_1_4_24_TransientBacklog_DoesNotStarveFreshIntent(t *testing.T) {
	...
	// The fresh row MUST be delivered despite the stuck backlog. Pre-fix this
	// never happens (ORDER BY id keeps re-claiming the stuck low-id rows).
