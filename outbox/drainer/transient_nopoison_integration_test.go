...
// Integration tests for sub-phase 1.4 D-5 (transient-class no-poison) + the
// mandatory CAS-claim exactly-once -race test (1.4-21).
//
// RED-phase (test-first, ban #12): asserts behaviour that the current drainer
// does NOT have — a long transient outage (> MaxAttempts consecutive failures)
// must NOT poison the intent. Today `attempt_count++` on every transient claim
// drives the row past the CAS gate (attempt_count < MaxAttempts) → permanent
// tuple loss. These tests fail until the transient-no-poison fix lands.
//
// Run: go test ./outbox/... -race -p 1
//   DOCKER_HOST=unix:///Users/dkot/.colima/default/docker.sock
...
// Test_1_4_04_LongTransientOutage_NoPoison — acceptance 1.4-04.
//
...
// Test_1_4_05_PermanentError_PoisonAndSurface — acceptance 1.4-05 / 1.4-04 contrast.
//
...
// Test_1_4_21_CASClaimExactlyOnce_Race — acceptance 1.4-21 (ban #10, MANDATORY -race).
//
