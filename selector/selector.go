...
// Соответствует §7 «02-data-model-and-conventions.md».
//
...
func BuildFrom(selectors []Selector, startParam int) (BuildResult, error) {
	...
	for _, sel := range selectors {
		...
		if len(andParts) == 0 {
			// Пустой selector без условий — матчит всё (эквивалент WHERE TRUE).
			// Если хотя бы один selector матчит всё — весь WHERE пуст.
			return BuildResult{}, nil
		}
