...
// OPA-overlay context.user.*) sees the wrong identity. KAC-127 Bug-2 fixed
// the ProjectService.Get peer-call path with a per-repo copy of this helper
// (`internal/clients/iam_client.go::withPrincipalMD` in kacho-vpc and
// kacho-compute) but did not touch the authz Check path. W1.4 lifts the
// helper here so any future peer-client (loadbalancer, dns) gets correct
// propagation by import.
//
// # Security boundary (workspace CLAUDE.md §запрет 6)
//
...
// `x-kacho-principal-*` headers (see
// `kacho-api-gateway/internal/middleware/auth.go:138-155` + `:307-319`) so a
// tenant cannot inject a principal header to impersonate another user. The
// server-side cluster-internal listener is mTLS-only and not reachable from
// outside the cluster, so it does not need a symmetric strip today. If that
// guarantee weakens, add a strip-on-entry in vpc/compute server interceptors
