...
// TestPropagateOutgoing_WithPrincipal_AppendsMD — W1.4 §3.1 PROP-CORELIB-01:
// ctx with Principal → outgoing MD carries all three x-kacho-principal-*
...
// TestPropagateOutgoing_EmptyPrincipal_PassThroughUnchanged — W1.4 §3.2
// PROP-CORELIB-02: fresh ctx with no Principal → no MD added, and the
// returned ctx must not panic on consumption.
//
// operations.PrincipalFromContext fallback'ит на SystemPrincipal() для пустого
// ctx; чтобы убедиться что mehlpper НЕ форсит headers для системного
...
// идёт через PrincipalFromContext → возвращает SystemPrincipal() (Type=system,
...
// TestSystemPrincipalFor_FormatsCorrectly — W1.4 §3.9 PROP-SYS-PRINCIPAL-01:
// builds attributable system principal for worker/reconciler contexts.
