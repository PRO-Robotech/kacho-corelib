...
-- KAC-105 (epic KAC-104 IAM DoD #4): добавить principal-поля в operations.
-- На E0/без auth заполняется stub'ом 'system'/'bootstrap'/'System' через
-- DEFAULT — это же значение использует operations.SystemPrincipal() и
-- operations.PrincipalFromContext(emptyCtx). На E2+ auth-interceptor
