...
// Unit-тесты durable terminal-write (срез E, стадия S1): retry/backoff на
...
type flakyRepo struct {
	...
	// markDoneFailsLeft / markErrorFailsLeft — сколько первых попыток упадёт.
...
// E1-03: первая попытка MarkDone падает transient → worker ретраит → success.
...
// E1-04: DB недоступна дольше budget → MarkDone не проглочен (failures++),
// строка done НЕ записана (reconciler-backstop добьёт позже).
func TestWorker_MarkDone_PermanentFail_NotSwallowed(t *testing.T) {
	...
	assert.False(t, ok, "при невосстановимом сбое строка НЕ помечается done (остаётся для reconciler)")
	assert.GreaterOrEqual(t, rec.TerminalWriteRetries("MarkDone"), float64(1),
		"перед сдачей было >=1 retry")
}

// E1-05: panic в worker-fn → recover → MarkError(INTERNAL "internal worker
...
// E1-06 (b): MarkError симметрично MarkDone — retry на transient-сбое.
