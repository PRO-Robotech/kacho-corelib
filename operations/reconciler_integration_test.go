...
// Integration-тесты (testcontainers Postgres) startup-reconciler'а (срез E,
// стадия S3): осиротевшие in-flight операции разрешаются в терминал по
...
// E3-01: restart mid-Create, ресурс закоммичен → MarkDone(current resource).
...
// E3-02: restart mid-Create, writer-TX откатилась (ресурс отсутствует) →
...
// E3-03: restart mid-Delete, ресурс удалён → MarkDone(Empty).
...
// E3-04: restart mid-Delete, ресурс ещё жив → MarkError(interrupted).
...
// E3-05: grace-окно защищает live-worker (свежая done=false строка не eligible).
func TestReconciler_GraceWindow_SkipsLiveWorker(t *testing.T) {
	...
	require.NoError(t, err)
	assert.False(t, got.Done, "live-worker не разрешён преждевременно")
}

// E3-06: два reconciler-движка поверх одной DB — exactly-once через SKIP LOCKED.
func TestReconciler_TwoReplicas_SkipLocked_ExactlyOnce(t *testing.T) {
	...
	// Каждый orphan разрешён ровно один раз: сумма recovered == числу orphan'ов.
...
// E3-07: reconciler добивает невосстановимый MarkDone-fail (E1-04 backstop).
func TestReconciler_RecoversPermanentTerminalWriteFail(t *testing.T) {
	...
	// Имитация строки из E1-04: done=false после исчерпания ретраев, ресурс
	// фактически закоммичен (resolver вернёт Done), modified_at старше grace.
...
// E3-08: restart mid-Update, ресурс закоммичен → MarkDone(current committed).
...
// E3-«restart»: повторное создание Reconciler'а поверх ТОЙ ЖЕ DB (имитация
