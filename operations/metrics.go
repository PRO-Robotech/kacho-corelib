...
// НЕ импортируется (corelib остаётся dependency-light): сервис подключает
...
//	operations_orphans_recovered_total{outcome}   — orphan'ы, разрешённые reconciler'ом (done|error)
...
// IncTerminalWriteRetries инкрементит счётчик ретраев терминальной записи.
...
// IncTerminalWriteFailures инкрементит счётчик невосстановимых терминальных записей.
...
// IncOrphansRecovered инкрементит счётчик разрешённых reconciler'ом orphan'ов.
...
// IncReconcileRuns инкрементит счётчик прогонов sweep-цикла.
...
// IncReconcileErrors инкрементит счётчик ошибок sweep-цикла.
...
// ReconcileErrors возвращает счётчик ошибок sweep-цикла.
