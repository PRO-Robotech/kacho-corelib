...
// interruptedMsg — frozen INTERNAL-текст для orphan'а, не довёдшего работу до
// commit. Намеренно отличается от worker-сбоя ("internal worker error"): оператор
// по тексту отличает live-worker-сбой от reconciler-разрешённого orphan'а.
...
const (
	...
	// остаётся done=false, sweep повторится позже.
	...
	// Create/Update либо ещё жив для Delete) → MarkError(interrupted).
	OutcomeInterrupted
)
...
// операции определяет её терминальный исход, сверяясь с committed-реальностью
...
type ReconcilerConfig struct {
	...
	// разрешать преждевременно ещё-живого worker'а. Дефолт 5m.
...
// проходе). Зовётся на старте ДО приёма трафика.
